...
async def deploy(client: AccountClient, player_address: int) -> int:
    print("[+] deploying sh256")
    riddle_deployment = await Contract.deploy(
        client=client,
        compiled_contract=Path("compiled/sha256.cairo").read_text(),
        constructor_args=[],
    )
...
async def checker(client: AccountClient, sha_contract: Contract, player_address: int) -> bool:
    solution = (await sha_contract.functions["is_challenge_done"].call()).res
